/** File Toolkit.java - a class with general tools, version 06, 2015-07-21

   This class is a "pure" class and does not have a main program. It does 
   not require any other classe; it is stand-alone
   ...
   padString      Pad a string on the left, right or both. Preserve longer string
                     Two parameter lists (overloaded)

   padStringTruncate
                  Pad a string on the left, right or both. Truncate longer string
                     Two parameter lists (overloaded)
  ...
  */
   ...
   */
   ...
   public static String giveMeAPlural(int quantity, String suffix) {
      ...
      String s = suffix;      // Assume quantity != 1
      if (quantity == 1) { s = ""; }
      return s;
   } // End giveMeAPlural
   ...
   */    
   ...
   /** padStringTruncate, 4 parameters
       Pad a string on the left and/or right side; truncate to 'width'
       ...
       Process    1. If the width is too small, give it the default length
                  2. If the string is longer than the width, truncate it
                     to 'width' characters
                  3. Pad the string on the left and/or right
       ...
       Note       To pad on only one side, say the left, use the empty
                  ...
                  If BOTH the left and right padding strings are NOT null,
                  the resulting string will end up longer than 'width', so
                  it will be truncated to 'width' characters
      ...
      Examples    The tilde (~) character is used to denote a space.
                  ...
                  padString(name, 8, "", " ")               John~~~~
                  padString(name, 8, " ", "")               ~~~~John
                  padString(name, 8, " ", " ")              ~~~~John  (truncated)
                  padString(name, 8, "", "*")               John****
   */    
    
   public static String padStringTruncate(String str, 
                                          int width, 
                                          String padLeft,
                                          String padRight) {
      ...
      // Using the length of the String str, calculate the number of characters
      // to pad on the left. Note the number could be <= 0 in which case no 
      // padding is done
      charsToPad = width - strPad.length();
      ...
      // If the string is now longer than 'width' characters, truncate it
      // to 'width' characters
      if (strPad.length() > width) {
         strPad = strPad.substring(0,width); // Does 0..width-1
      } // End if
   ...
   /** padStringTruncate, 2 parameters
       Pad a string on the right side with blanks; truncate if needed
       ...
       Process    Use padString with 4 parameters, an empty string for the
                  right pad parameter and a blank for the left pad
                  parameter, so the string is left-padded with blanks.
                  Truncate strings that are longer than 'width'
   ...
   */    
   ...
   public static String padStringTruncate(String str, int width) {
                                    
      String strPad;         // String to be returned
      strPad = padStringTruncate(str, width, "", " ");
      return strPad;
   } // end padString, 2 parameters
